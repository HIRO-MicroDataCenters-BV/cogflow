...
def _group_gates_by_layer(
    ...
    for layer_i, channel_c, raw in zip(layer_indices, channel_indices, raws, strict=True):
...
def _build_layer_lora(
    ...
    for slot, (channel_c, delta) in enumerate(gated_channels):
        # Bounds-check before indexing; a raw IndexError from torch is
        # opaque ("index 17 is out of bounds for dimension 0 with size
        # 4") whereas this surface mismatches between the controller
        # state and the base model weights (e.g. a controller trained
        # against a wider hidden_dim) with a usable message.
        if not 0 <= channel_c < d_out:
            raise ValueError(
                f"Gated channel index {channel_c} is out of range for the "
                f"target module's d_out={d_out}. The controller state and "
                "the base model's weight shape don't match — check that "
                "the controller was trained against this base."
            )
...
def controller_to_lora(
    *,
    ...
    try:
        ...
        raise ImportError(
            "controller_to_lora requires torch. Install with "
            "`pip install torch` (or `pip install cogflow[llm]` "
            "which pulls torch + safetensors)."
        ) from exc
    ...
    # Detach + move every LoRA tensor to CPU before writing. Fine-tune
    # callers commonly pass ``weight_lookup`` closures that return CUDA
    # tensors (the base model is already on-GPU at training time), and
    # ``_build_layer_lora`` builds A/B inheriting ``weight.device``.
    # ``safetensors.torch.save_file`` requires CPU tensors and
    # ``torch.save`` would otherwise persist a device-specific pickle
    # that won't load on a CPU-only host. Detach also strips any
    # autograd graph the caller might have left attached.
    ...
    if save_safetensors:
        try:
            ...
            raise ImportError(
                "save_safetensors=True requires the safetensors package. "
                "Install with `pip install safetensors` (or "
                "`pip install cogflow[llm]` which pulls torch + safetensors)."
            ) from exc
        save_file(cpu_state_dict, str(out_dir / "adapter_model.safetensors"))
    else:
        torch.save(cpu_state_dict, out_dir / "adapter_model.bin")
